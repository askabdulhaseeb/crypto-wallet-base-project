    ...
    try {
      ...
      for (int i = 0; i < json.length; i++) {
        CoinData coinData = CoinData(
          name: json[i]['name'] ?? '',
          daypercentage:
              json[i]['market_data']['price_change_percentage_24h'].toDouble() ?? 0,
          imageurl: json[i]['image']['small'] ?? '',
          marketcaprank: json[i]['market_data']['market_cap_rank'].toDouble() ?? 0,
          price: json[i]['market_data']['current_price']['usd'].toDouble() ?? 0,
          symbol: json[i]['symbol'] ?? '',
          weekpercentage:
              json[i]['market_data']['price_change_percentage_7d'].toDouble() ?? 0,
        );
